...
class StockPerteController extends StockBaseController
{
    public function index(Request $request): JsonResponse
    {
        ...
        $query = PerteProduit::with(['produit', 'utilisateurSignale'])
            ->where('entreprise', $entreprise->id);

        if ($motif !== '') {
            $query->where('motif_perte', 'ilike', '%' . $motif . '%');
        }

        if ($dateDebut) {
            $query->where('date_perte', '>=', $dateDebut);
        }

        if ($dateFin) {
            $query->where('date_perte', '<=', $dateFin);
        }

        $total = $query->count();

        $pertes = $query->orderByDesc('date_perte')
            ->forPage($page, $limit)
            ->get()
            ->map(function (PerteProduit $perte) use ($entreprise) {
                $annulee = DB::table('historiques')
                    ->where('module', 'stock')
                ...
                return [
                    'id'                 => $perte->id,
                    'quantite_perdu'     => (float) $perte->quantite_perdu,
                    'motif_perte'        => $perte->motif_perte,
                    'date_perte'         => $perte->date_perte,
                    'annulable'          => !$annulee && Carbon::parse($perte->date_perte)->addHours(24)->isFuture(),
                    'produit'            => $perte->produit ? [
                        'id'    => $perte->produit->id,
                        'nom'   => $perte->produit->nom,
                        'image' => $perte->produit->image,
                    ] : null,
                    'utilisateur'        => $perte->utilisateurSignale ? [
                        'id'     => $perte->utilisateurSignale->id,
                        'nom'    => $perte->utilisateurSignale->name,
                        'prenom'  => $perte->utilisateurSignale->prename,
                    ] : null,
